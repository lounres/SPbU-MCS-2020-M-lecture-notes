...
\begin{document}
    ...
    \begin{definition}
        Пусть $X \subseteq \AA^n$ и $Y \subseteq \AA^m$ --- замкнутые подмножества. Временно обозначим $t_i := T_i|_X \in k[X]$ --- координатная функция на $X$. Отображение $\varphi: Y \to X$ называется \emph{регулярной}, если $t_i \circ \varphi \in k[Y]$ (т.е. каждая координата $\varphi$ как отображение является регулярной).
    \end{definition}
    ...
    \begin{lemma}
        Пусть $X$, $Y$, $Z$ --- аффинные \todo{Видимо, замкнутые?} множества, а $\varphi: Y \to X$ и $\psi: X \to Z$ регулярны. Тогда $\psi \circ \varphi: Y \to Z$ регулярны.
    \end{lemma}
    ...
    \begin{remark}
        ...
        \[\Max(B) = \Max(k[X]) = \text{точки из $X$} = X.\]
        Формальнее, $\Max(k[X]) = \{I(\{x\})\}_{x \in X}$. Тогда мы можем рассмотреть $i_X: k \to k[X], a \mapsto (x \mapsto a)$ и для всякого $x^*: k[X] \to k, f \mapsto f(x)$. Тогда $x^* \circ i_X = \Id_k$.

        Назовём гомоморфизм $s: k[X] \to k$ $k$-алгебр \emph{сечением} вложения $i_X$, если $s \circ i_X = \Id_k$. Тогда $\Ker(s) \in \Max(k[X]) = \{\mathcal{M}_x\}_{x \in X}$. Тогда у нас есть биекция между сечениями вложений $i_X$ и точками $X$.
    \end{remark}

    Рассмотрим следующую конструкцию. Пусть $\varphi: X' \to X$ --- регулярное отображение. Тогда есть $\psi: X' \to X$, которое переводит $x'$ в такое $x$, что $\varphi^*(s) = s'$, где $s$ и $s'$ --- сечения, соответствующие $x$ и $x'$.
